...
Установить [Python](https://www.python.org/downloads/) версии 3.9.
...
## Перед запуск сервера - использование менеджера настроек ##
...
Для проверки работы сервера запустите мини-клиента `client.py` в консоли:

```cmd
pipenv run python -i ./client.py
```

После запуска клиент отправит на сервер сообщение об авторизации, ответ сервера будет выведен в консоль, после чего `python` перейдёт в режим интерактивной строки, для того чтобы была возможность провести дополнительные проверки.
