...
class Error:
    ...
    def catching_error(self, status: str,
                       add_info: Exception | str = None) -> api.Errors:
        """Сatches errors in "try...except" content.
        Result is class 'api.Errors' with information about code, status,
        ...
        Returns:
            dict: returns class 'api.Errors' according to protocol,
                like: {
                    'code': 200,
                    'status': 'Ok',
                    'time': 123456545,
...
class User(Error):
    def check_auth(method_to_decorate):
        def wrapper(self, request):
            ...
            auth_id = request.data.user[0].uuid
            try:
...
class ProtocolMethods(User, Error):
    ...
    def __init__(self, request):
        self.jsonapi: api.Version = api.Version(version=api.VERSION)
        self.get_time: int = int(time())
        self.response: api.ValidJSON = None
        self.request: api.ValidJSON = None

        try:
            self.request = api.Request.parse_obj(request)
            logger.success("Validation was successful")
        except ValidationError as ERROR:
            self._errors("UNSUPPORTED_MEDIA_TYPE",
                         str(ERROR))
    ...
    def get_response(self,
                     response: api.ValidJSON = None) -> json:
    ...
    def _register_user(self,
        ...
        if self.check_login(login):
            ...
            models.UserConfig(uuid=uuid,
                              ...
                              authId=auth_id)
            user.append(api.User(uuid=uuid,
                                 auth_id=auth_id))
            data = api.Data(time=self.get_time,
                            user=user)
    ...
    def _get_update(self,
        ...
        if dbquery_message.count():
            for element in dbquery_message:
                result = api.Message(uuid=element.uuid,
                                     text=element.text,
        ...
        if dbquery_flow.count():
            for element in dbquery_flow:
                result = api.Flow(uuid=element.uuid,
        ...
        if dbquery_user.count():
            for element in dbquery_user:
                result = api.User(uuid=element.uuid,
    ...
    def _send_message(self,
        ...
        try:
            ...
            models.Message(uuid=message_uuid,
                           ...
                           user=user,
                           flow=flow)
            message.append(api.Message(uuid=message_uuid,
                                       client_id=client_id,
                                       from_flow=flow_uuid,
                                       from_user=user_uuid))
            data = api.Data(time=self.get_time,
    ...
    def _all_messages(self,
        ...
        try:
            ...
            if MESSAGE_COUNT <= limit.getint("messages"):
                flow.append(api.Flow(uuid=flow_uuid,
        ...
        if users_volume <= limit.getint("users"):
            for element in request.data.user[1:]:
                try:
                    ...
                    result = api.User(uuid=dbquery.uuid,
                                      login=dbquery.login,
    ...
    def _authentification(self,
        ...
        if self.check_login(login):
            ...
            if generator.check_password():
                ...
                user.append(api.User(uuid=dbquery.uuid,
